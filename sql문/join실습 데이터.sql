...
CREATE TABLE customers (
  ...
  phone VARCHAR(20),
  birth_date DATE,
  membership_level VARCHAR(20)  -- 예: Silver, Gold, VIP
);
CREATE TABLE orders (
  order_id INT PRIMARY KEY AUTO_INCREMENT,
  customer_id INT,
  order_date DATE,
  order_status VARCHAR(20),      -- 예: '처리중', '배송중', '완료'
  payment_method VARCHAR(30),    -- 예: '신용카드', '계좌이체', '카카오페이'
...
CREATE TABLE products (
  ...
  category VARCHAR(50),     -- 예: 전자기기, 생활용품, 의류 등
  manufacturer VARCHAR(50)  -- 제조사 정보
);
